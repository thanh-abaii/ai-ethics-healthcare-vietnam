...
G2, `SCREENING_EXTRACTION_CODEBOOK_GATE`, `PRISMA_SCR_PROTOCOL_MAP_GATE`, chiến lược tìm kiếm chính xác, bộ chuẩn quốc tế, competitive checkpoint, khai báo tài trợ và đăng ký OSF công khai đều đã có bằng chứng đạt yêu cầu. `TEAM_EXPERTISE_GATE` là `NOT_APPLICABLE_SCOPE_NARROWED` theo `PR-02` và không còn là dependency. `PRISMA_SCR_MANUSCRIPT_GATE` không phải dependency tiền đăng ký; cổng này chỉ chặn nộp bài. G4–G5 vẫn `NOT_RUN` nhưng đã được phép bắt đầu.
