...
4. **Tệp hồ sơ kiểm toán bản ghi thô (Raw Audit Trail CSV):**  
   ...
   `artifacts/search-run-2026-07-31/official-raw-harvest-and-deduplication-audit-trail.csv`
...
1. **Sàng lọc Vòng 1:** `artifacts/search-run-2026-07-31/official-screening-workspace-round-1.csv`
2. **Trích xuất Vòng 2:** `artifacts/search-run-2026-07-31/official-data-extraction-workspace-round-2.csv`
3. **Kiểm toán Bản ghi thô:** `artifacts/search-run-2026-07-31/official-raw-harvest-and-deduplication-audit-trail.csv`
4. **Phản hồi API OpenAlex gốc:** `artifacts/search-run-2026-07-31/openalex/`
5. **Script cập nhật tự động:** `scripts/update_human_approvals.py`

### Lệnh kiểm tra nhanh cho Codex (PowerShell):
```powershell
python -c "import csv; r=list(csv.DictReader(open('artifacts/search-run-2026-07-31/official-screening-workspace-round-1.csv', encoding='utf-8'))); print('Total:', len(r)); print('Passed:', len([x for x in r if x['screening_status_round_1']=='PASSED_TO_ROUND_2'])); print('Excluded:', len([x for x in r if x['screening_status_round_1']=='EXCLUDED_ROUND_1']))"
```
